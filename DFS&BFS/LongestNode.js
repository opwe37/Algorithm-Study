...
function solution(n, edge) {
  var answer = 0;
  var edges = Array(n).fill(0).map(val => new Array);
  for (let [row, col] of edge) {
    edges[row-1].push(col-1);
    edges[col-1].push(row-1);
  }
  ...
  while (possibleNode.length != 0) {
    ...
    for (let i = 0; i < loop; i++) {
      ...
      for (let col of edges[node]) {
        if (isVisit[col]) continue;
